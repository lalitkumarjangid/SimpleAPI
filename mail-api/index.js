 ...
 * Response:
 * {
 *   "results": [{ "email": "a@b.com", "legit": true, "reason": "mailbox verified" }],
...
app.post("/verify", async (req, res) => {
  try {
    ...
    const response = results.map(({ email, legit, reason }) => ({
      email,
      legit,
      reason,
    }));

    const legitCount = response.filter((r) => r.legit).length;
